# AI-01 — Agent Orchestration Specification

**Project:** AI Vendor Selection System  
**Dokumen:** AI-01 — Agent Orchestration  
**Versi:** 3.0.0  
**Tanggal:** 2026-06-13  
**Status:** Draft  
**Author:** —  
...
Dokumen ini **tidak** mendefinisikan algoritma scoring TOPSIS, detail prompt, atau mekanisme RAG secara mendalam — itu masing-masing ada di AI-03, AI-02, dan AI-05.
...
| AI-02 | Prompt Library | Template prompt yang digunakan tiap agent |
| AI-03 | Scoring Engine | Dipanggil oleh Orchestrator setelah semua agent selesai |
| AI-05 | RAG Specification | Pipeline indexing dokumen yang dipicu saat upload |
| AI-06 | Qualitative Analyzer Agent | Spesifikasi agent keenam |
| AI-07 | Preference Matcher Agent | Spesifikasi agent ketujuh |
...
**Apa yang dianalisis:**
- Unique offerings yang disebutkan dalam dokumen penawaran (via RAG context dari AI-05)
...
**Output yang dihasilkan:** Narasi `profil_kualitatif` per vendor dan `summary_komparatif` antar semua vendor — tidak ada skor numerik. Detail lengkap di AI-06.
...
**Output yang dihasilkan:** Narasi rekomendasi berbasis preferensi (atau framing netral), analisis kesesuaian tiap vendor, dan catatan konflik jika rekomendasi preferensi bertentangan dengan ranking TOPSIS. Detail lengkap di AI-07.
...
Orchestrator mengagregasi semua output agent
        ↓
Kirim data agregat ke Scoring Engine (AI-03)
        ↓
...
**Qualitative Analyzer** — menerima tambahan RAG context dari AI-05: potongan relevan dari dokumen penawaran tiap vendor yang diambil menggunakan query pencarian unique offerings.
...
Pipeline bercabang — dua proses berjalan bersamaan:
  ...
  CABANG B — Indexing RAG (detail di AI-05):
...
**Output agent harus selalu dalam format terstruktur yang terdefinisi.** LLM diminta untuk menghasilkan output dalam format JSON yang spesifik (didefinisikan di AI-02 dan AI-06/AI-07). Output yang tidak sesuai format dianggap sebagai kegagalan partial dan dicatat.
...
Setiap agent membutuhkan minimal dua prompt: system prompt yang mendefinisikan peran dan output format, serta user prompt template yang menyertakan data vendor dan requirement. AI-02 perlu mendefinisikan kedua prompt untuk ketujuh agent. Qualitative Analyzer dan Preference Matcher memiliki karakteristik prompt yang berbeda karena outputnya naratif, bukan JSON terstruktur.

### Untuk AI-03 (Scoring Engine)

Orchestrator memanggil Scoring Engine setelah semua agent selesai. AI-03 perlu mendefinisikan format data agregat yang diharapkan — termasuk output dari Qualitative Analyzer dan Preference Matcher sebagai input tambahan untuk reasoning naratif.

### Untuk AI-05 (RAG Specification)

Pipeline indexing RAG berjalan sebagai bagian dari proses ekstraksi dokumen (section 9). AI-05 mendefinisikan detail teknis chunking, embedding, dan penyimpanan ke pgvector yang dipicu dari pipeline ini.
...
| 2.0.0 | 2026-06-11 | Tambah Qualitative Analyzer Agent (AI-06) dan Preference Matcher Agent (AI-07); perbarui orchestration flow menjadi tujuh agent; perbarui pipeline ekstraksi dokumen untuk menyertakan RAG indexing (AI-05) | — |
...
| 4.0.0 | 2026-06-13 | Rename dari BE-03 ke AI-01 (ADR-035); perbarui semua referensi kode dokumen: BE-04→AI-02, BE-05→AI-03, BE-08→AI-05, BE-09→AI-06, BE-10→AI-07 | — |
