# BE-09 — Qualitative Analyzer Agent Specification

**Project:** AI Vendor Selection System  
**Dokumen:** BE-09 — Qualitative Analyzer Agent  
**Versi:** 1.0.0  
**Tanggal:** 2026-06-11  
**Status:** Draft  
**Author:** —  
...
Dokumen ini **tidak** mendefinisikan implementasi kode atau detail teknis orchestration — itu ada di BE-03. Dokumen ini fokus pada **apa** yang dilakukan agent ini dan **mengapa** ia perlu ada.
...
|---|---|---|
| BE-03 | Agent Orchestration | Posisi agent dalam alur orchestration |
| BE-04 | Prompt Library | Template prompt agent ini (perlu diperbarui) |
| BE-05 | Scoring Engine | Integrasi output kualitatif ke dalam hasil akhir |
| BE-08 | RAG Specification | Sumber data dokumen yang digunakan agent ini |
...
Performance Scorer
        ...
        └── Qualitative Analyzer ───────────────┘
                ↓ keduanya selesai
        Preference Matcher (BE-10)
                ↓ selesai
        Scoring Engine (BE-05)
```
...
**Mengapa sebelum Scoring Engine:** Output Qualitative Analyzer Agent dibutuhkan oleh Scoring Engine untuk disertakan dalam reasoning naratif hasil akhir, dan oleh Preference Matcher (BE-10) untuk pencocokan preferensi.
...
Jika indexing dokumen gagal atau tidak selesai untuk vendor tertentu (BE-08 section 12.1 dan 12.2), agent tetap berjalan menggunakan output dari agent lain sebagai sumber utama. Dalam kasus ini:
...
Mengikuti konvensi BE-04 section 8.2:
...
Scoring Engine (BE-05) mengonsumsi output Qualitative Analyzer Agent untuk dua tujuan:

**Reasoning naratif** — `profil_kualitatif` dan `summary_komparatif` disertakan sebagai input ke LLM yang menghasilkan reasoning naratif akhir (BE-05 section 9). Ini memperkaya penjelasan dengan dimensi kualitatif yang tidak tertangkap oleh angka skor.
